...
public class Gui extends Application {
    ...
    public void start(Stage stage) {
        ...
        historyButton.setOnAction(event -> {
            ...
            Scene historyScence= new Scene(listView);
            for (IMemento memento : controller.getAllHistory()) {
                listView.getItems().add( memento.getInfo());
            }


            Stage historyStage = new Stage();
            historyStage.setScene(historyScence);
            historyStage.setTitle("History");
            historyStage.show();
            listView.setOnMouseClicked(event1 -> {
                int index = listView.getSelectionModel().getSelectedIndex();
                if (index >= 0) {
                    IMemento selectedMemento = controller.getAllHistory().get(index);
